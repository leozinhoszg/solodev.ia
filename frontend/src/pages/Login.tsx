...
export default function Login() {
  ...
  const slideFrom = (location.state as { slideFrom?: "center" } | null)
    ?.slideFrom;
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      // Iris closes → navigates to / → iris opens on the dashboard
      navigateWithIris("/");
    } catch (err: any) {
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // Continuous float + glow pulse on logo
      ...
      // Iris pulse — stays in place, gentle opacity fade
      gsap.fromTo(
        "#logo-iris",
        { opacity: 0.55 },
        {
      ...
      // Wordmark letters — stagger reveal
      gsap.fromTo(
        ".logo-letter",
        { opacity: 0, y: 4 },
        {
      ...
      // Gradient sweep — shift the linearGradient endpoints
      gsap.fromTo(
        "#logo-grad",
        { attr: { x1: -80, x2: 120 } },
        {
      ...
      if (subtitleRef.current) {
        subtitleRef.current.textContent = "Entre para continuar sua jornada";
      }
      tl.fromTo(
        subtitleRef.current,
        { opacity: 0 },
  ...
  function handleGoToRegister(e: React.MouseEvent) {
    e.preventDefault();
    ...
    gsap.to(cardRef.current, {
      ...
      duration: 0.35,
      ease: "power2.in",
      onComplete: () => navigate("/register", { state: { slideFrom: "center" } }),
    });
  }
